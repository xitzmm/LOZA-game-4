...
const IMG_PATHS = {
  ...
  loza: 'assets/loza_pixel_final.png',
  ...
  obstacle: 'assets/obstacle_pixel.png',
  ahmed: 'assets/ahmed_small.png'
};
const assets = {};
let loadPromises = [];
...
function buildLevel(){
  ...
  // obstacle tile width (consistent)
  const obsW = 160; const obsH = 48;
  // starting platform: size = 2 * obsW, connected to first obstacle (adjacent)
  ...
  const plats = [
    {x:360,y:380,w:220},{x:640,y:340,w:180},{x:920,y:300,w:150},{x:1160,y:340,w:200},
    {x:1420,y:300,w:140},{x:1680,y:340,w:180},{x:1940,y:320,w:160},{x:2200,y:360,w:220},
    {x:2560,y:320,w:160},{x:2820,y:360,w:180},{x:3120,y:300,w:220},{x:3440,y:340,w:220}
  ];
  ...
  // pills (Ahmed Taha) smaller sizes and patrol ranges on some platforms
  pills = [
    {x:startPlat.x + 80, y:startPlat.y - 40, w:40, h:40, range:[startPlat.x + 20, startPlat.x + startPlat.w - 40], speed:70, dir:1, blinkTimer:0},
    {x:680,y:268,w:36,h:36, range:[640,760], speed:90, dir:1, blinkTimer:0},
    {x:1500,y:328,w:36,h:36, range:[1480,1640], speed:80, dir:1, blinkTimer:0},
    {x:1900,y:288,w:36,h:36, range:[1840,2060], speed:60, dir:1, blinkTimer:0},
    {x:2500,y:328,w:36,h:36, range:[2420,2780], speed:100, dir:1, blinkTimer:0},
    {x:3100,y:268,w:36,h:36, range:[2920,3300], speed:80, dir:1, blinkTimer:0},
  ];
  ...
  // heart position (final), safe platform
  heart = {x:3700,y:284,w:56,h:56};
...
function render(){
  ...
  // draw sun and clouds (parallax layers)
  if(assets.sun){
    // sun fixed in sky (parallax slower than camera)
    ctx.drawImage(assets.sun, 760 - cameraX*0.1, 20, 128, 128);
  }
  if(assets.cloud){
    // multiple clouds moving slowly
    for(let i=0;i<5;i++){
      const cx = (i*220 + (Date.now()/50 % 220)) - cameraX*0.2;
      ctx.drawImage(assets.cloud, cx, 40 + (i%2)*10, 192, 96);
    }
  }
  ...
  // draw obstacles (pixel tiles)
  for(let o of obstacles){
    if(assets.obstacle) ctx.drawImage(assets.obstacle, o.x, o.y, o.w, o.h);
    else { ctx.fillStyle = '#8b0000'; ctx.fillRect(o.x, o.y, o.w, o.h); }
  }

  // draw pills (Ahmed Taha) with small text above (smaller size)
  for(let m of pills){
